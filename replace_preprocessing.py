"""Run: python replace_preprocessing.py"""
...
with open("utils/data_preprocessing.py", "w") as f:
    f.write(new_code)
print("Written!")
...
print(f"\nCalifornia: {len(ca)} rows")
print(f"Sales sample: {ca['Sales'].head(5).tolist()}")
if ca["Sales"].max() > 0:
    print("\n*** SUCCESS - Now run: python run.py --train-only ***")
else:
    print("\n*** STILL BROKEN ***")